...
function toFeed(e, today) {
  ...
  /* Photographs only. A bio row's picture is sometimes a DESIGNED CARD — its
     own headline, its own CTA, sometimes a date — which works at 60px in a
     link list and fails completely as a full-bleed 1920px TV panel, where it
     becomes a second poster arguing with the board (and keeps saying whatever
     it said the day it was drawn). `imageIsCard` opts those out at the source
     rather than the feed guessing from a file extension. */
  const pool = (Array.isArray(e.images) && e.images.length ? e.images : [e.image])
    .filter(Boolean)
    .map(src => (/^https?:/.test(src) ? src : ORIGIN + src.replace(/^\/+/, '')));
  if (!e.imageIsCard && pool.length) {
    out.image = pool[0];
    /* The whole pool travels, so a board showing several nights of one series
       can give each of them a different frame instead of the same one twice. */
    if (pool.length > 1) out.images = pool;
  }

  /* A featured event may carry motion. Same folder as the pictures, same
     deploy; the still stays on as the poster so nothing depends on it. */
  if (!e.imageIsCard && e.video)
    out.video = /^https?:/.test(e.video) ? e.video : ORIGIN + e.video.replace(/^\/+/, '');

  /* A day-programme, when the event has one. The board turns it into its own
     screen on the day itself. */
  if (e.programme && Array.isArray(e.programme.blocks) && e.programme.blocks.length)
    out.programme = e.programme;

  /* Marks an event for the rotation's closing run rather than the paged walk
     — see _highlights below. */
  if (e.highlight) out.highlight = true;

  if (e.url) out.url = e.url;
  /* The organiser's exact price wording. A caption-less screen has to carry
     the fact that decides attendance (D-023 · D-049) — and never a member-only
     price without its condition (D-092 · D-094), which is why this is copied
     from the card, not assembled here. */
  if (e.price) out.price = e.price;
...
const payload = {
  ...
  _highlights:
    'highlight:true takes an event OUT of the board\'s paged walk and gives it a run of its own at the end of every lap. For the productions and festivals that would otherwise sit months down a chronological list and never reach a screen. Each event still appears exactly once per lap.',
  _video:
    'video (optional, per event) is an mp4 shown in place of the still on the featured panel and the full-screen slide — muted, looping, with the still as its poster. Compress before publishing (D-030) and keep it short; the board loops it. A viewer with reduced-motion set, or a file that fails to load, sees the still.',
  _images:
    'image is the frame this event leads with; images (when present) is the whole pool it may draw from, so a board showing several nights of one series can avoid repeating a photograph. A bio row flagged imageIsCard is a designed graphic rather than a photograph and is published with no image at all — the board then draws its own flat placeholder.',
  _times:
